...
{
    public class Orden_OrdenFCIDTO_profile : Profile
    {
        public Orden_OrdenFCIDTO_profile()
        {
            CreateMap<Orden, OrdenFCIDTO>().ForMember(dest => dest.ID, origen => origen.MapFrom(mapa => mapa.ID))
                ...
                .ForMember(dest => dest.Activo, origen => origen.MapFrom(mapa => mapa.Activo.Nombre))
